...
tf.compat.v1.app.flags.DEFINE_integer('batch_size', 3, """Number of images to process in a batch.""")
tf.compat.v1.app.flags.DEFINE_string('data_dir', '', """Path to the data directory.""")
tf.compat.v1.app.flags.DEFINE_boolean('use_fp16', False, """Train the model using fp16.""")
tf.compat.v1.app.flags.DEFINE_string('weights', 'fp_weights.npy', """Path to the Pre-trained weights.""")
tf.compat.v1.app.flags.DEFINE_float('wd', 0.00025, """Number of images to process in a batch.""")
tf.compat.v1.app.flags.DEFINE_float('dropout', 0.5, "Define the dropout ratio")
tf.compat.v1.app.flags.DEFINE_boolean('projection', True, """Projection layer or zero padding.""")
tf.compat.v1.app.flags.DEFINE_boolean('use_pretrained', True, """Whether to use the pre-trained weights or not.""")
tf.compat.v1.app.flags.DEFINE_float('learning_rate', 0.0001, """Initial learning rate.""")
tf.compat.v1.app.flags.DEFINE_float('lr_decay_epochs', 350, """Number of epochs per learning rate decay.""")
...
def res_block(scope, prev_layer, conv_shapes, stride, train, atrous=0):
    # branch2a
    with tf.compat.v1.variable_scope('branch2a') as scope_inner:
        branch2a = conv_relu(scope_inner, scope.name + '_branch2a', prev_layer, conv_shapes[0], stride, train, atrous)

    # branch2b
    with tf.compat.v1.variable_scope('branch2b') as scope_inner:
        branch2b = conv_relu(scope_inner, scope.name + '_branch2b', branch2a, conv_shapes[1], 1, train, atrous)

    # branch2c
    with tf.compat.v1.variable_scope('branch2c') as scope_inner:
        kernel = _variable_with_weight_decay('weights', shape=conv_shapes[2], wd=FLAGS.wd,
                                             layer_name=scope.name + '_branch2c')
        if atrous > 0:
    ...
    if (input_depth != output_depth) & (FLAGS.projection == True):
        with tf.compat.v1.variable_scope('branch1') as scope_inner:
            kernel = _variable_with_weight_decay('weights', shape=[1, 1, input_depth, output_depth], wd=FLAGS.wd,
                                                 layer_name=scope.name + '_branch1')
...
def inference(images, ignore, possibleLabels, train=False, rep=False):
    ...
    # conv3_4
    with tf.compat.v1.variable_scope('conv3_4') as scope:
    ...
    with tf.compat.v1.variable_scope('concat') as scope:
    ...
    # fuse
    with tf.compat.v1.variable_scope('fuse') as scope:
    ...
    with tf.compat.v1.variable_scope('softmax_linear') as scope:
        weights = _variable_with_weight_decay('weights', [1, 1, 512, NUM_CLASSES], wd=FLAGS.wd,
                                              layer_name=scope.name + '_w')
