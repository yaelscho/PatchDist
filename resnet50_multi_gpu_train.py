...
tf.app.flags.DEFINE_boolean('start_new_phase', False, """Whether this is the first phase of the run or not.""")
...
def tower_loss(scope, images, labels, edges, ignore, poss_lbls):
    ...
    logits, _, fuse = resnet50.inference(images, ignore, poss_lbls, train=True)
...
def train():
    with tf.Graph().as_default(), tf.device('/cpu:0'):
            ...
            for i in xrange(FLAGS.num_gpus):
                with tf.device('/gpu:%d' % i):
                    with tf.name_scope('%s_%d' % (resnet50.TOWER_NAME, i)) as scope:
                        ...
                        # all towers.
                        loss, fuse = tower_loss(scope, image_batch, label_batch, edges_batch, ignore_batch, poss_lbls_batch)
        ...
        if FLAGS.use_pretrained & FLAGS.new_run:
            model_dict = np.load(FLAGS.weights, allow_pickle=True).item()
        ...
        for step in xrange(int(step_reached) + 1, max_steps):
            ...
            _, loss_value, image_batch_, label_batch_, edges_batch_, ignore_batch_, poss_lbls_batch_ = sess.run(
                [train_op, loss, image_batch, label_batch, edges_batch, ignore_batch, poss_lbls_batch])
