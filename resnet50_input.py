...
def distorted_inputs(data_dir, batch_size):
    ...
    read_input.label = tf.image.resize_images(read_input.label, [new_height_and_width[0], new_height_and_width[1]],
                                              method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    read_input.edge = tf.image.resize_images(read_input.edge, [new_height_and_width[0], new_height_and_width[1]],
                                             method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
    ...
    distorted_image = tf.cond(tf.equal(randint, 1), lambda: tf.image.flip_left_right(reshaped_image),
                              lambda: reshaped_image)
    read_input.label = tf.cond(tf.equal(randint, 1), lambda: tf.reverse(read_input.label, [1]),
                               lambda: read_input.label)
    ...
    print(
        'Filling queue with %d BSDS images before starting to train. This will take a few minutes.' % min_queue_examples)
...
def test_inputs(data_dir):
    filenames_ = os.listdir(data_dir + 'BSDS500/data/images/test/')
    filenames_.sort()
